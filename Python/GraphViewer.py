...
class GraphViewer(QtGui.QMainWindow):
    ...
    def __init__(self, data_buffer_length):
        ...
        # Appearance settings:
        self.pen_style = pg.mkPen(
            color="y", width=2, style=QtCore.Qt.SolidLine)
        self.title_color = "w"
        ...
        self.SDS_unit = "\u00B5g/m\u00B3"
        self.PPD_unit = "ppL"
        self.names_units = OrderedDict(
            [
                ('Temperature', self.C_label),
                ('Pressure', 'Pa'),
                ('Humidity', '%'),
                ...
                ('Particle concentration', self.PPD_unit),
                ('Particle data valid', '')
            ]
        )
    ...
    def setParticleUnits(self, name):
        if (name == "SDS011"):
            ...
            raise Exception(
                "Particle sensor name must be 'SDS011' or 'PPD42', or None")
    ...
    # Fill the ComboBoxes with the list of items and set the initial selected
    # values
    ...
    def updateGraphs(self):
        for n in range(0, len(self.plot_handles)):
            ...
            if (self.graph_var_numbers[n] >= len(self.indices)):
                ...
                if not (self.is_bar_chart[n] is True):
                    self.plot_items[n].removeItem(self.plot_handles[n])
                    self.plot_handles[n].deleteLater()
                    self.plot_handles[n] = pg.BarGraphItem(
                        x=list(range(0, self.sound_band_number)),
                ...
                new_data = [
                    self.data_buffer[i][-1] for i in range(
                        self.band1_index,
                ...
                if not (self.is_bar_chart[n] is False):
                    self.plot_items[n].removeItem(self.plot_handles[n])
                    self.plot_handles[n].deleteLater()
                    self.plot_handles[n] = self.plot_items[n].plot(
                ...
                self.plot_items[n].setTitle(
                    list(self.names_units.keys())[ind]
                    + " = {:.{dps}f} ".format(
                        self.data_buffer[self.graph_var_numbers[n]][-1],
                        dps=self.decimal_places[ind]
                    ) + list(self.names_units.values())[ind],
                    color=self.title_color,
                    size=self.title_size
                )
                self.plot_handles[n].setData(
                    self.time_data,
                    self.data_buffer[self.graph_var_numbers[n]]
                )
    ...
    # Format the bar chart for displaying sound data for the six frequency
    # bands
    ...
    def formatBarChart(self, item):
        ...
        # X axis ticks: set minor to same as major and label according to
        # frequency
        x_ticks = [[0] * len(frequency_midpoints), [0]
                   * len(frequency_midpoints)]
