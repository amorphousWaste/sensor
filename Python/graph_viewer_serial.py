...
class GraphViewerSerial(GraphViewer):
    ...
    def __init__(self, buffer_length, serial_port):
        ...
        self.line_count = 0
        # There are 4 input cases resulting from the use of cycle_readout.ino
        # and on_demand_readout.ino, with 15, 18, 19 and 22 data columns.
        # These lists define which variables are present in each case:
        self.col_indices = []
        # no air quality or particle data
        self.col_indices.append(list(range(0, 4)) + list(range(8, 19)))
        # no air quality data
        self.col_indices.append(list(range(0, 4)) + list(range(8, 22)))
        # no particle data
        self.col_indices.append(list(range(0, 19)))
        self.col_indices.append(
            list(range(0, 22)))                    # all data
        # the index of 'Band 1 SPL' in the four lists
        self.sound_band1_index = [7, 7, 11, 11]
