...
# CYCLE_PERIOD_3_S, CYCLE_PERIOD_100_S, CYCLE_PERIOD_300_S, or None
...
class GraphViewerI2C(GraphViewer):
    ...
    def __init__(self, buffer_length, cycle_period, OD_delay_ms):
        ...
        if (cycle_period is not None):
            ...
            # Use on-demand mode with any chosen time delay between
            # measurements
            ...
            if (self.delaying):
                ...
                if ((time_now_ms - self.time_start_ms) >= self.OD_delay_ms):
                    ...
                    self.I2C_bus.write_byte(
                        i2c_7bit_address, ON_DEMAND_MEASURE_CMD)
                    self.delaying = False
            else:
                if GPIO.event_detected(READY_pin):
                    self.readData()
    ...
    def putDataInBuffer(
        self, air_data, air_quality_data, light_data, sound_data, particle_data
    ):
        i = 0
        i = self.appendData(
            i,
        ...
        i = self.appendData(
            ...
            + [
                ...
                for i in range(0, self.sound_band_number)
