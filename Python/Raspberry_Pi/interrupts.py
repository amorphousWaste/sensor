...
# The interrupt threshold value in lux units can be fractional and is formed
# as:
...
if (
    (light_thres_lux_i + (float(light_thres_lux_f2dp) / 100.0)) > MAX_LUX_VALUE
):
...
if (enable_sound_interrupts):
    ...
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, SOUND_INTERRUPT_TYPE_REG, [sound_int_type])

    # Set the threshold
    setSoundInterruptThreshold(I2C_bus, sound_thres_mPa)

    # Tell the Pi to monitor the interrupt line for a falling edge event
    # (high-to-low voltage change)
    GPIO.add_event_detect(sound_int_pin, GPIO.FALLING)

    # Enable the interrupt on the MS430
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, SOUND_INTERRUPT_ENABLE_REG, [ENABLED])


if (enable_light_interrupts):
    # Set the interrupt type (latch or comparator)
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, LIGHT_INTERRUPT_TYPE_REG, [light_int_type])

    # Set the threshold
    setLightInterruptThreshold(
        I2C_bus, light_thres_lux_i, light_thres_lux_f2dp)

    # Set the interrupt polarity
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, LIGHT_INTERRUPT_POLARITY_REG, [light_int_polarity])
    ...
    # Enable the interrupt on the MS430
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, LIGHT_INTERRUPT_ENABLE_REG, [ENABLED])
