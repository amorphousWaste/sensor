...
# IFTTT settings:
IFTTT_url = "http://maker.ifttt.com/trigger/" + \
    IFTTT_EVENT_NAME + "/with/key/" + WEBHOOKS_KEY
...
while (True):
    ...
    for v in [temperature, humidity, air_quality_index]:
        ...
        if send_alert:
            ...
            try:
                value1 = "The " + v['name'] + \
                    " is too " + threshold_description
                print("Sending new alert to IFTTT: " + value1)
                payload = {
                    "value1": value1,
                    "value2": (
                        "The measurement was {:.{dps}f} ".format(
                            v['data'], dps=v['decimal_places']
                        ) + v['unit']
                    ),
                    "value3": advice
                }
                requests.post(
                    IFTTT_url, json=payload, headers=IFTTT_header, timeout=2
                )
            except Exception as e:
                # An error has occurred, likely due to a lost internet
                # connection, and the post has failed.
                # The program will continue and new
