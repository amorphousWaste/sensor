...
from sensor_functions import (
    ...
    format_light_data,
    format_sound_data,
    format_particle_data,
)
from unicode_constants import UnicodeConstants


class SimpleWebpageHandler(http.server.SimpleHTTPRequestHandler):
    """Make a simple text web page showing the environment data.

    Data is shown in separate category tables, using HTML and CSS.
    This is used in web_server.py
    """
    ...
    def assemble_web_page(cls):
        ...
        if cls.air_data:
            ...
            cls.the_web_page += (
                '<tr><td>Temperature</td><td id="v1">'
                '{:.1f}</td><td>'.format(cls.air_data['T'])
                + cls.air_data['T_unit'] + '</td></tr>'
            )
            cls.the_web_page += (
                '<tr><td>Pressure</td>'
                '<td id="v1">{}</td><td>Pa</td></tr>'.format(
                    cls.air_data['P_Pa']
                ) +
                '<tr><td>Humidity</td><td id="v1">{:.1f}</td><td>%</td></tr>'
                .format(cls.air_data['H_pc']) +
                '<tr><td>Gas Sensor Resistance</td><td id="v1">{}</td>'
                .format(cls.air_data['G_ohm']) +
                '<td>' + UnicodeConstants.ohm + '</td></tr></table></p>'
            )

        if cls.air_quality_data:
            cls.the_web_page += "<p><h2>Air Quality Data</h2>"
            if (cls.air_quality_data['AQI_accuracy'] == 0):
                # values are not valid yet
                cls.the_web_page += (
                    "<a>" + interpret_AQI_accuracy(
                        cls.air_quality_data['AQI_accuracy']
                    ) + "</a></p>"
                )
            else:
                cls.the_web_page += (
                    "<table><tr><td>Air Quality Index</td>"
                    "<td id='v2'>{:.1f}</td><td></td></tr>".format(
                        cls.air_quality_data['AQI']
                    ) +
                    "<tr><td>Air Quality Summary</td><td id='v2'>" +
                    interpret_AQI_value(cls.air_quality_data['AQI'])
                    + "</td><td></td></tr>"
                    "<tr><td>Estimated CO"
                    + UnicodeConstants.subscript_2
                    + "</td>"
                    "<td id='v2'>{:.1f}</td><td>ppm</td></tr>".format(
                        cls.air_quality_data['CO2e']
                    )
                    + "<tr><td>Equivalent Breath VOC</td><td id='v2'>{:.2f}"
                    .format(cls.air_quality_data['bVOC'])
                    + "</td><td>ppm</td></tr></table></p>"
                )

        if cls.sound_data:
            cls.the_web_page += (
                "<p><h2>Sound Data</h2><table>"
                "<tr><td>A-weighted Sound Pressure Level</td><td id='v3'>"
                "{:.1f}</td><td>dBA</td></tr>".format(
                    cls.sound_data['SPL_dBA']
                )
            )
            for i in range(0, SOUND_FREQ_BANDS):
                cls.the_web_page += (
                    "<tr><td>Frequency Band "
                    "{} ({} Hz) SPL</td>".format(
                        i + 1, sound_band_mids_Hz[i]
                    ) +
                    "<td id='v3'>{:.1f}</td><td>dB</td></tr>".format(
                        cls.sound_data['SPL_bands_dB'][i]
                    )
                )
            cls.the_web_page += (
                "<tr><td>Peak Sound Amplitude</td>"
                "<td id='v3'>{:.2f}</td><td>mPa</td></tr></table></p>".format(
                    cls.sound_data['peak_amp_mPa']
                )
            )

        if cls.light_data:
            cls.the_web_page += (
                "<p><h2>Light Data</h2><table>"
                "<tr><td>Illuminance</td><td id='v4'>{:.2f}</td>".format(
                    cls.light_data['illum_lux']
                )
                + "<td>lux</td></tr>"
                "<tr><td>White Light Level</td><td id='v4'>{}</td>".format(
                    cls.light_data['white']
                )
                + "<td></td></tr></table></p>"
            )

        if cls.particle_data:
            cls.the_web_page += (
                "<p><h2>Air Particulate Data</h2><table>"
                "<tr><td>Sensor Duty Cycle</td>"
                "<td id='v5'>{:.2f}</td><td>%</td></tr>".format(
                    cls.particle_data['duty_cycle_pc']
                ) + "<tr><td>Particle Concentration</td>"
                "<td id='v5'>{:.2f}".format(
                    cls.particle_data['concentration'])
                + "</td><td>" + cls.particle_data['conc_unit']
                + "</td></tr></table></p>"
            )
...
class GraphWebpageHandler(http.server.SimpleHTTPRequestHandler):
    ...
    This uses the Plotly.js libray, javascript, HTML and CSS.
    This is used in graph_web_server.py
    """
    ...
    def send_all_data(self):
        ...
        # so the web page knows when to do the next request
        self.wfile.write(struct.pack('H', self.data_period_seconds))
        # Send temperature unit and particle sensor type,
        # combined into one byte
        codeByte = PARTICLE_SENSOR
        if USE_FAHRENHEIT:
            codeByte = codeByte | 0x10
        self.wfile.write(struct.pack('B', codeByte))
        # Send the length of the data buffers
        # (the number of values of each variable)
        self.wfile.write(struct.pack('H', len(self.temperature)))
        # Send the data:
        for p in [
            self.AQI, self.temperature, self.pressure, self.humidity,
    ...
    def send_latest_data(self):
        """Send latest data."""
        self.wfile.write(bytes(self.data_header, "utf8"))
        # Send the most recent value for each variable,
    ...
    @classmethod
    def set_webpage_filename(self, filename: str):
        self.webpage_filename = filename

    @classmethod
    def set_buffer_length(cls, buffer_length: int):
